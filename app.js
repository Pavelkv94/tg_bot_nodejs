...
const botAction = () => {
    bot.on('message', async (msg) => {
        ...
        if (text === '/start') {
            await bot.sendMessage(chatId, `Приветствую тебяб ${msg.from.first_name}! Я твой личный ассистент. Пока нахожусь в демо-версии поэтому могу тупить)`)
    ...
    bot.on('callback_query', async msg => {
        ...
        if (data === chats[chatId]) {
            ...
            console.log(chats)
            await bot.sendMessage(chatId, `Вы не угадали!!, я загадал цифру ${chats[chatId]}`, againOptions)
        }
